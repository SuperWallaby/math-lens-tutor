...
const _maxCacheEntries = 160;

final _mixedMathWidgetCache = <String, Widget>{};

String _mixedMathCacheKey({
  required String text,
  required TextStyle style,
  required TextAlign textAlign,
  required bool paragraphSoftBreak,
  required bool readableSolutionStep,
...
class MixedMathText extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final key = _mixedMathCacheKey(
      text: text,
      style: style,
      textAlign: textAlign,
      paragraphSoftBreak: paragraphSoftBreak,
      readableSolutionStep: readableSolutionStep,
      preserveWhitespace: preserveWhitespace,
    );
    final cached = _mixedMathWidgetCache[key];
    if (cached != null) return cached;

    final built = _buildMixedMathWidget(
      text: text,
      style: style,
      textAlign: textAlign,
      paragraphSoftBreak: paragraphSoftBreak,
    ...
    if (_mixedMathWidgetCache.length >= _maxCacheEntries) {
      _mixedMathWidgetCache.clear();
    }
    _mixedMathWidgetCache[key] = built;
    return built;
  }
}
